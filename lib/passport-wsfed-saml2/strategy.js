...
WsFedSaml2Strategy.prototype._authenticate_jwt = function (req, state) {
  ...
  var reqSecToken = doc.getElementsByTagNameNS('http://schemas.xmlsoap.org/ws/2005/02/trust', 'RequestedSecurityToken');
  let data = reqSecToken[0].textContent;
  let buff = new Buffer(data, 'base64');  
  let token = buff.toString('ascii');

  console.log(`Token: ${token}`);
  console.log(`Cert: ${this.options.cert}`);
